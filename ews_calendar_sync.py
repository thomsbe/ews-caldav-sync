...
parser.add_argument(
    "--config", default="config.toml", help="Path of the TOML configuration file"
)
...
ews_credentials = Credentials(
    username=config["ews"]["username"], password=config["ews"]["password"]
)
ews_config = Configuration(server=config["ews"]["server"], credentials=ews_credentials)
ews_account = Account(
    config["ews"]["account"],
    access_type=IMPERSONATION if config["ews"]["impersonate"] else DELEGATE,
    config=ews_config,
)
...
dav_calendar: caldav.Calendar = dav_principal.calendar(
...
fetch_ids = []
for change_type, item in ews_calendar.sync_items(
    sync_state=sync_state, only_fields=["uid", "id", "changekey"]
):
    if change_type in ("create", "update"):
        ...
        try:
            ...
            logger.error(
                "Error while fetching deleted item %s from CalDAV server:",
                item,
                exc_info=e,
            )
            continue
...
for item in ews_account.fetch(ids=fetch_ids):
    if isinstance(item, CalendarItem):
        if tag_ignore and item.categories and tag_ignore in item.categories:
            logger.info("Skipping item %s due to category tag", item.subject)
            continue

        if item.subject in fixed_ignore:
            logger.info("Skipping item %s due to fixed ignore list", item.subject)
            continue
        ...
        if not item.mime_content:
            ...
            continue
        try:
            data: icalendar.Calendar = icalendar.Calendar.from_ical(
                item.mime_content.decode("utf-8")
            )
            del data["method"]
            event_data = next(
                iter(c for c in data.subcomponents if isinstance(c, icalendar.Event))
            )
